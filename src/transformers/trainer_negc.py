...
path = os.path.join(package_directory, "models", model_ver, "best-negc-for-intervention", 
                    model_ver, model_ver_to_negc_folder[model_ver])
...
NEGC = load_conceptor(os.path.join(path, f"layer-12.pkl")
...
class Trainer:
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        ##Added by Yifei
        if USE_POST_PROCESS:
            print("Compute loss with conceptor post processing.")
            outputs = outputs @ NEGC
        ## End added by Yifei
